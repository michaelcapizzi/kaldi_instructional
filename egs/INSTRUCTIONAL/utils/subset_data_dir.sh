...
function do_filtering {
  ...
  [ -f $srcdir/feats.scp ] && ${KALDI_INSTRUCTIONAL_PATH}/utils/filter_scp.pl $destdir/utt2spk <$srcdir/feats.scp >$destdir/feats.scp
  [ -f $srcdir/vad.scp ] && ${KALDI_INSTRUCTIONAL_PATH}/utils/filter_scp.pl $destdir/utt2spk <$srcdir/vad.scp >$destdir/vad.scp
  [ -f $srcdir/utt2lang ] && ${KALDI_INSTRUCTIONAL_PATH}/utils/filter_scp.pl $destdir/utt2spk <$srcdir/utt2lang >$destdir/utt2lang
  [ -f $srcdir/utt2dur ] && ${KALDI_INSTRUCTIONAL_PATH}/utils/filter_scp.pl $destdir/utt2spk <$srcdir/utt2dur >$destdir/utt2dur
  [ -f $srcdir/wav.scp ] && ${KALDI_INSTRUCTIONAL_PATH}/utils/filter_scp.pl $destdir/utt2spk <$srcdir/wav.scp >$destdir/wav.scp
  [ -f $srcdir/spk2warp ] && ${KALDI_INSTRUCTIONAL_PATH}/utils/filter_scp.pl $destdir/spk2utt <$srcdir/spk2warp >$destdir/spk2warp
  [ -f $srcdir/utt2warp ] && ${KALDI_INSTRUCTIONAL_PATH}/utils/filter_scp.pl $destdir/utt2spk <$srcdir/utt2warp >$destdir/utt2warp
  [ -f $srcdir/text ] && ${KALDI_INSTRUCTIONAL_PATH}/utils/filter_scp.pl $destdir/utt2spk <$srcdir/text >$destdir/text
  [ -f $srcdir/spk2gender ] && ${KALDI_INSTRUCTIONAL_PATH}/utils/filter_scp.pl $destdir/spk2utt <$srcdir/spk2gender >$destdir/spk2gender
  [ -f $srcdir/cmvn.scp ] && ${KALDI_INSTRUCTIONAL_PATH}/utils/filter_scp.pl $destdir/spk2utt <$srcdir/cmvn.scp >$destdir/cmvn.scp
  if [ -f $srcdir/segments ]; then
     ${KALDI_INSTRUCTIONAL_PATH}/utils/filter_scp.pl $destdir/utt2spk <$srcdir/segments >$destdir/segments
     ...
     [ -f $srcdir/wav.scp ] && ${KALDI_INSTRUCTIONAL_PATH}/utils/filter_scp.pl $destdir/reco <$srcdir/wav.scp >$destdir/wav.scp
     [ -f $srcdir/reco2file_and_channel ] && \
       ${KALDI_INSTRUCTIONAL_PATH}/utils/filter_scp.pl $destdir/reco <$srcdir/reco2file_and_channel >$destdir/reco2file_and_channel
     
     # Filter the STM file for proper sclite scoring (this will also remove the comments lines)
     [ -f $srcdir/stm ] && ${KALDI_INSTRUCTIONAL_PATH}/utils/filter_scp.pl $destdir/reco < $srcdir/stm > $destdir/stm
...
if $spk_list_specified; then
  mkdir -p $destdir
  ${KALDI_INSTRUCTIONAL_PATH}/utils/filter_scp.pl "$spk_list" $srcdir/spk2utt > $destdir/spk2utt || exit 1;
  ${KALDI_INSTRUCTIONAL_PATH}/utils/spk2utt_to_utt2spk.pl < $destdir/spk2utt > $destdir/utt2spk || exit 1;
  do_filtering; # bash function.
  exit 0;
elif $utt_list_specified; then
  mkdir -p $destdir
  ${KALDI_INSTRUCTIONAL_PATH}/utils/filter_scp.pl "$utt_list" $srcdir/utt2spk > $destdir/utt2spk || exit 1;
  ${KALDI_INSTRUCTIONAL_PATH}/utils/utt2spk_to_spk2utt.pl < $destdir/utt2spk > $destdir/spk2utt || exit 1;
  do_filtering; # bash function.
  exit 0;
elif $speakers; then
  mkdir -p $destdir
  ${KALDI_INSTRUCTIONAL_PATH}/utils/shuffle_list.pl < $srcdir/spk2utt | awk -v numutt=$numutt '{ if (tot < numutt){ print; } tot += (NF-1); }' | \
    sort > $destdir/spk2utt
  ${KALDI_INSTRUCTIONAL_PATH}/utils/spk2utt_to_utt2spk.pl < $destdir/spk2utt > $destdir/utt2spk
  do_filtering; # bash function.
  exit 0;  
elif $perspk; then
  mkdir -p $destdir
  ...
  if $shortest; then
    ...
    ${KALDI_INSTRUCTIONAL_PATH}/utils/filter_scp.pl $destdir/tmp.uttlist $srcdir/utt2spk >$destdir/utt2spk
    rm $destdir/tmp.uttlist $destdir/tmp.len
  else
    ${KALDI_INSTRUCTIONAL_PATH}/utils/subset_scp.pl $first_opt $numutt $srcdir/utt2spk > $destdir/utt2spk || exit 1;
  fi
  ${KALDI_INSTRUCTIONAL_PATH}/utils/utt2spk_to_spk2utt.pl < $destdir/utt2spk > $destdir/spk2utt
  do_filtering;
  exit 0;
fi
