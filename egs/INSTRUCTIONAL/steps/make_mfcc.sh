...
nj=2
cmd=run.pl
mfcc_config=${KALDI_INSTRUCTIONAL_PATH}/conf/mfcc.conf
compress=true
...
if [ -f ${KALDI_INSTRUCTIONAL_PATH}/path.sh ]; then . ${KALDI_INSTRUCTIONAL_PATH}/path.sh; fi
...
${KALDI_INSTRUCTIONAL_PATH}/utils/validate_data_dir.sh --no-text --no-feats $data || exit 1;
...
for n in $(seq $nj); do
  ...
  ${KALDI_INSTRUCTIONAL_PATH}/utils/create_data_link.pl $mfccdir/raw_mfcc_$name.$n.ark
done
...
# handling a segmented transcript
  ...
  ${KALDI_INSTRUCTIONAL_PATH}/utils/split_scp.pl $data/segments $split_segments || exit 1;
  ...
  ${KALDI_INSTRUCTIONAL_PATH}/utils/split_scp.pl $scp $split_scps || exit 1;
